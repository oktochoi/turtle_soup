    ...
    try {
      ...
      try {
        ...
        if (solveCountsError && (solveCountsError.message || solveCountsError.code || solveCountsError.details)) {
          ...
          if (solvesError) {
            console.error('문제 맞춘 수 직접 집계 오류:', solvesError);
            setSolveRanking([]);
          } else if (solvesData) {
            const userSolveMap = new Map<string, number>();
            solvesData.forEach((solve) => {
              if (solve.user_id) {
                userSolveMap.set(solve.user_id, (userSolveMap.get(solve.user_id) || 0) + 1);
              }
            });

            // 각 사용자가 만든 문제의 author 필드를 조회하여 닉네임 가져오기
            const userIds = Array.from(userSolveMap.keys());
            const { data: problemsData } = await supabase
              .from('problems')
              .select('user_id, author')
              .in('user_id', userIds);
            
            // user_id별로 가장 최근 author 값 사용
            ...
            const solveRankingArray = Array.from(userSolveMap.entries())
            ...
            setSolveRanking(solveRankingArray);
          } else {
            setSolveRanking([]);
          }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400">랭킹을 불러오는 중...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white">
      <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-6 max-w-4xl">
        ...
        <div className="text-center mb-6 sm:mb-8">
          ...
          <p className="text-slate-400 text-sm sm:text-base">문제 맞춘 수와 받은 하트를 확인하세요</p>
        </div>
          ...
          <div className="space-y-3">
            {solveRanking.length === 0 ? (
              <div className="text-center py-12 bg-slate-800/50 rounded-xl border border-slate-700">
                ...
                <p className="text-slate-400">아직 문제를 맞춘 사용자가 없습니다.</p>
              </div>
            ) : (
              solveRanking.map((user, index) => {
                ...
                return (
                  <div
                    key={user.user_id}
                    className={`bg-gradient-to-r ${getRankColor(rank)} rounded-xl p-4 sm:p-5 border transition-all hover:scale-[1.02]`}
                  >
                    <div className="flex items-center justify-between">
                        ...
                        <div>
                          <h3 className="text-lg sm:text-xl font-bold text-white mb-1">
                            {user.nickname || (user.email 
                              ? user.email.split('@')[0] 
                              : `사용자 ${user.user_id.substring(0, 8)}...`)}
                          </h3>
                      ...
                      <div className="text-right">
                        <div className="text-2xl sm:text-3xl font-bold text-teal-400 flex items-center justify-end gap-1">
                          <i className="ri-checkbox-circle-fill"></i>
                          {user.solve_count}
                        </div>
                        <div className="text-xs sm:text-sm text-slate-400">맞춘 문제</div>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="space-y-3">
          {problemRanking.length === 0 ? (
            ...
            problemRanking.map((author, index) => {
              ...
              return (
                <div
                  key={author.user_id || `author_${index}`}
                  className={`bg-gradient-to-r ${getRankColor(rank)} rounded-xl p-4 sm:p-5 border transition-all hover:scale-[1.02]`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs sm:text-sm text-slate-300">
                          문제 {author.problem_count}개 작성
                        </p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-2xl sm:text-3xl font-bold text-purple-400 flex items-center justify-end gap-1">
                        <i className="ri-heart-fill text-pink-400"></i>
                        {author.total_likes}
                      </div>
                      <div className="text-xs sm:text-sm text-slate-400">받은 하트</div>
                    </div>
                  </div>
                </div>
              );
