...
const CATEGORIES: Category[] = [
  { id: 'all', label: '전체', icon: 'ri-list-check', color: 'from-slate-500 to-slate-600' },
  { id: 'notice', label: '공지사항', icon: 'ri-megaphone-line', color: 'from-red-500 to-pink-500' },
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 max-w-6xl">
        ...
        {posts.length === 0 ? (
          <div className="text-center py-12 bg-slate-800/50 rounded-xl border border-slate-700">
            ...
            <p className="text-slate-400 mb-4">아직 게시글이 없습니다.</p>
            {user && (
              <Link href="/community/create">
              ...
              <div className="text-center py-12 bg-slate-800/50 rounded-xl border border-slate-700">
                ...
                <p className="text-slate-400 mb-4">
                  {selectedCategory === 'all' 
                    ? '아직 게시글이 없습니다.' 
                    : `${getCategoryInfo(selectedCategory).label} 게시판에 게시글이 없습니다.`}
                </p>
                {user && (
                  <Link href="/community/create">
                    <button className="px-4 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white rounded-lg transition-all">
                      ...
                      <div className="flex items-center gap-4">
                        {postGameUserIds.get(post.id) ? (
                          <div
                            onClick={(e) => {
                              e.stopPropagation();
                              router.push(`/profile/${postGameUserIds.get(post.id)}`);
                            }}
