  ...
  const handleCheckIn = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        if (gameUser) {
          ...
          const { data: newGameUser } = await supabaseClient
            ...
            .insert({
              ...
              nickname: user.user_metadata?.full_name || user.email?.split('@')[0] || `사용자${user.id.substring(0, 8)}`,
            })
            .select()
            .single();
        ...
        if (gameUser) {
          ...
          const { data: newGameUser } = await supabaseClient
            ...
            .insert({
              guest_id: guestId,
              nickname: `게스트${guestId.substring(0, 6)}`,
            })
            .select()
            .single();
      ...
      if (result && result.success) {
        setIsCheckedIn(true);
        setCheckInMessage(`출석 완료! +${result.gainedXP} XP, +${result.gainedPoints} P 획득!`);
        
        // 레벨업/업적/칭호 알림 (간단한 alert)
        if (result.leveledUp) {
          setTimeout(() => {
            alert(`🎉 레벨업! 레벨 ${result.newLevel} 달성!`);
          }, 500);
        }
        if (result.unlockedAchievements.length > 0) {
          setTimeout(() => {
            alert(`🏆 업적 달성: ${result.unlockedAchievements.map(a => a.name).join(', ')}`);
          }, 1000);
        }
        if (result.unlockedTitles.length > 0) {
          setTimeout(() => {
            alert(`👑 칭호 획득: ${result.unlockedTitles.map(t => t.name).join(', ')}`);
          }, 1500);
        }
      } else {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12 max-w-md lg:max-w-5xl relative z-10">
        ...
        <section className="mt-8 sm:mt-12 lg:mt-16 space-y-6">
          ...
          {isLoadingProblem ? (
            ...
            <Link href={`/problem/${todayProblem.id}`}>
            ...
            <div className="bg-slate-800/40 backdrop-blur-xl rounded-2xl p-5 sm:p-6 lg:p-7 border border-slate-700/50 hover:border-purple-500/50 transition-all cursor-pointer animate-fade-in-up delay-100">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-300 text-sm sm:text-base">정답 랭킹과 문제 좋아요 랭킹을 확인하세요</p>
                </div>
